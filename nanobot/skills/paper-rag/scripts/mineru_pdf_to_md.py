...
from workspace_paths import DEFAULT_RAG_ROOT, load_workspace_env, resolve_pdf_path, resolve_rag_root, RAG_BASE
...
def trace_enabled() -> bool:
    return os.getenv("PAPER_RAG_TRACE", os.getenv("RAGANYTHING_TRACE", "1")).lower() not in {"0", "false", "no"}


def trace_event(event: str, **fields) -> None:
    trace_path = os.getenv("PAPER_RAG_TRACE_FILE") or os.getenv("RAGANYTHING_TRACE_FILE")
...
def sync_db(root: Path) -> None:
    ...
    paper_rag = Path(__file__).with_name("paper_rag.py")
    if not paper_rag.exists():
        return
    completed = subprocess.run([sys.executable, str(paper_rag), "--root", str(root), "sync", "--mode", "fast"], check=False)
    if completed.returncode != 0:
        print(f"Paper RAG sync skipped/failed with exit code {completed.returncode}", flush=True)


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--root", type=Path, default=DEFAULT_RAG_ROOT, help="RAG base or book folder (default: articles/rag).")
    parser.add_argument("--book", help="Book folder name under articles/rag/ (default: PDF stem).")
    ...
    pdf_path = resolve_pdf_path(RAG_BASE, args.pdf, book=args.book)
    root = resolve_rag_root(args.root, pdf=pdf_path, book=args.book)
    root.mkdir(parents=True, exist_ok=True)
    load_workspace_env(root)
    os.environ.setdefault("PAPER_RAG_TRACE_FILE", str(root / "paper_rag_trace.jsonl"))
    md_dir = root / "md"
    env = load_env(root / ".env")
    if not env:
        from workspace_paths import articles_root_for

        env = load_env(articles_root_for(root) / ".env")
    pdf_paths = [pdf_path]
